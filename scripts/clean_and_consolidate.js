...
function findFilePathByName(name) {
  // search DOWNLOADS_ROOT recursively for the filename
  try {
...
function buildOutputRow(rec) {
  // Input columns in pipeline CSV: company_name,year,scope,value,unit,source_file,extraction_method
  ...
  // Fix cases where year and value were swapped
  ...
  if ((!year || year === 'Unknown' || !isYear(year)) && parsedValue && isYear(parsedValue)) {
    ...
    // clear numeric value if it matched year
    valueRaw = '';
  }

  // If year holds a numeric large number that looks like value and value is empty, swap
  if (isYear(year) && (!valueRaw || valueRaw === 'Unknown')) {
    // keep year
  }

  const value = parseValue(valueRaw);

  // find source path to infer country
